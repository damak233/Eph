...
function FAQ({ magicEden, tensor }: { magicEden: string, tensor: string }) {
  const faqs = [
    ...
    { q: 'Will Save the Children post about the launch?', a: 'Their social media team does not do collaboration posts during active fundraising, but may consider posts after completion. They can comment from the central SCUK account.' },
  ]
  return (
...
function PreviewArtwork() {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden">
      <div className={`absolute inset-0 transition-opacity duration-700 ${expired ? 'opacity-0' : 'opacity-100'}`}>
        <div className="w-full h-full bg-[radial-gradient(circle_at_40%_40%,rgba(255,255,255,0.25),rgba(255,255,255,0.03))]" />
      </div>
      <div className={`absolute inset-0 grid place-items-center transition-opacity duration-700 ${expired ? 'opacity-100' : 'opacity-0'}`}>
        <div className="text-center">
          <div className="text-white/90 text-sm">“The light has faded.”</div>
          <div className="mt-2 text-white/60 text-xs">— memory remains</div>
        </div>
      </div>
    </div>
  )
